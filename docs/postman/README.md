...
-   If you prefer groups to remain even when they have no klasifikasi (empty arrays), set `only_with_children=false`.- To get a flat, per-type structure (example: lists per geo type like `klasifikasi_pola_ruang`, `klasifikasi_struktur_ruang`, etc.), use `format=flat&rtrw_id=<id>`.
